"""
S1.1-1.5B on GSM8K with small token budgets
"""
...
class S1SmallBudgetForcer:
    ...
    def generate_with_small_budget_forcing(self, question: str, min_thinking_tokens: int) -> Dict:
        ...
        if min_thinking_tokens > 256 and self.config.num_ignore_stops > 0:
            ...
            if max_tokens_thinking_remaining > 20: 
...
def main():
    ...
    config = BenchmarkConfig(
        ...
        min_token_budgets=[64, 128, 256, 384, 512], 
        max_samples=100,  
    ...
    if results:
        ...
        viable_results = [r for r in results if r.accuracy > 0.05]  
        if viable_results:
